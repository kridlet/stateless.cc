...
def team_confirmation():
    ...
    team_members = db.session.query(User).join(User.roles).filter(Role.name == 'member').filter(User.van == current_user.van, User.region==current_user.region).all()
   ...
   # ========================================= #
    ...
    # If the caller text "Activate", a response will be sent         
        ...
        try:
            ...
            if on_shift == True:
                client.messages.create(to=inbound_msg_from, from_=TWILIO_NUMBER,body="You are currently active. ")
            ...
            # status in the database to True and welcome them caller and ask for their Van Number  
                ...
                db.session.commit()
                client.messages.create(to=inbound_msg_from, from_=TWILIO_NUMBER,body="Welcome back, what is your FOUR digit van number? ")
    ...
    # The callers first name response is handled
        ...
        if inbound_msg_body.lower().replace(" ","")[:5] == "abort":
            session.pop('last_name_session')
            session.pop('activation_step')
            session.pop('activate_session')
            session.pop("add_van_session")

            client.messages.create(to=inbound_msg_from, from_=TWILIO_NUMBER,body="You have aborted the activation process.")
        ...
        # The callers first name response is handled
            ...
            if add_van: 
                db.session.query(User).filter(User.phone_number == inbound_msg_from).update({"region":(region), "van":(van)})
                db.session.commit()
                session.pop("add_van_session")
    ...
    elif inbound_msg_body.lower().replace(" ","")[:7] == "roster" and get_caller_role(inbound_msg_from) == "teamlead":
        
        user = get_caller_by_phone_number(inbound_msg_from) 
        roster = get_team(user.region, user.van)
        roster_list = []
        for x in roster:
            roster_list.append("%s %s" %(x.first_name, x.last_name))
